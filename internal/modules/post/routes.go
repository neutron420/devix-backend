...
func RegisterRoutes(rg *gin.RouterGroup, handler *Handler, jwtManager *jwtpkg.Manager) {
	...
	posts.Use(middleware.OptionalAuth(jwtManager))
	{
		...
		posts.GET("/:slug", handler.GetBySlug)
		posts.PUT("/:slug", middleware.Auth(jwtManager), handler.Update)
		posts.PATCH("/:slug/autosave", middleware.Auth(jwtManager), handler.Autosave)
		posts.DELETE("/:slug", middleware.Auth(jwtManager), handler.Delete)
		posts.POST("/:slug/media", middleware.Auth(jwtManager), handler.UploadMedia)
	}
